  ...
  def _Wrapper(*args: P.args, **kwargs: P.kwargs) -> None:
    try:
      ...
      # no context
        ...
        cli_logging.Console().print(
          str(err)
        )  # print only error message (DEBUG level is verbose already)
      else:
        cli_logging.Console().print_exception()  # print full traceback (less verbose mode needs it)
...
def GenerateTyperHelpMarkdown(
  ...
  *,
  ...
  for path, _, _ in _ClickWalk(click_root, root_ctx, []):
    ...
    cli_logging.ResetConsole()  # ensure clean state on each command, also raises on duplicate log
